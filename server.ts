...
reservasRouter.post('/reservas/cancelamento', async (req: Request, res: Response) => {
    try {
...
reservasRouter.put('/reservas/cancelamento/:id', async (req: Request, res: Response) => {
    try {
        const response = await axios.put(`http://localhost:3333/hoteis/${req.params.id}`, req.body);
        res.json(response.data);
    } catch (e) {
        res.status(500).json({ message: 'Error accessing the database', error: "teste" });
    }
